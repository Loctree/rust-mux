...
./target/release/rust_mux \
  --socket ~/.rmcp_servers/rust_mux/sockets/memory.sock \
  --cmd npx -- @modelcontextprotocol/server-memory \
  --max-active-clients 5 \
  --status-file ~/.rmcp_servers/rust_mux/status.json
# host side: point to bundled proxy
rust_mux_proxy --socket ~/.rmcp_servers/rust_mux/sockets/memory.sock
```
...
- `status`: check host configs point to `rust_mux_proxy`.
...
- Prefer `rust_mux_proxy` over `socat` for host STDIO integration.
